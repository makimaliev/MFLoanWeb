		...
		var datatable = $('#paymentsTable').mDatatable({
			...
			columns: [{
                ...
                template: function (row) {
                    ...
                    result = result + '\
						<a href="/manage/debtor/'+ debtorId + '/loan/'+ loanId + '/payment/' + row.id +'/save" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Редактировать">\
							<i class="la la-edit"></i>\
						</a>\
						<a href="/manage/debtor/'+ debtorId + '/loan/'+ loanId + '/payment/' + row.id +'/delete" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Удалить">\
