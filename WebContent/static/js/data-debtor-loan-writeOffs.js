		...
		var datatable = $('#WOsTable').mDatatable({
			...
			columns: [{
                ...
                template: function (row) {
                    ...
                    result = result + '\
						<a sec:authorize="hasAnyRole(ADMIN,PERM_UPDATE_WRITEOFF)" href="/manage/debtor/'+ debtorId + '/loan/'+ loanId + '/wo/' + row.id +'/save" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Редактировать">\
							<i class="la la-edit"></i>\
						</a>\
						<a hidden="hidden" href="/manage/debtor/'+ debtorId + '/loan/'+ loanId + '/wo/' + row.id +'/delete" class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" title="Удалить">\
