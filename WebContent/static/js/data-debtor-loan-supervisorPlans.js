		...
		var datatable = $('#SPsTable').mDatatable({
			...
			columns: [{
                ...
                template: function (row) {
                    ...
                    result = result + '\
						<a sec:authorize="hasAnyRole(ADMIN,PERM_UPDATE_SUPERVISORPLAN)" href="/manage/debtor/'+ debtorId + '/loan/'+ loanId + '/sp/' + row.id +'/save" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Редактировать">\
