        ...
        var datatable = $('#termsTable').mDatatable({
            ...
            columns: [{
                ...
                template: function (row) {
                    return '\
						<a href="/manage/debtor/'+debtorId+'/loan/'+loanId+'/term/'+row.id+'/view" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Просмотр"><i class="la la-arrow-circle-o-right"></i></a>\
					';
                }
            }, {
                field: "startDate",
                ...
                template: function (row) {
                    ...
                    if(isEmptyChild==true) {
                        result = result + '\
                            <a sec:authorize="hasAnyAuthority(ADMIN,PERM_UPDATE_ORDERTERM)" href="/manage/debtor/' + debtorId + '/loan/' + loanId + '/term/' + row.id + '/save" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Редактировать">\
