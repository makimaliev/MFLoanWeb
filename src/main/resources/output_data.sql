...
CREATE VIEW collection_phase_view AS
  ...
  FROM collection_procedure_view `cpv`,
    collectionPhase `cph`,
    region r,
    district d,
    workSector ws
  WHERE `cpv`.`v_cp_id` = `cph`.`collectionProcedureId` and cpv.v_debtor_region_id = r.id and cpv.v_debtor_district_id = d.id and cpv.v_debtor_work_sector_id = ws.id;
