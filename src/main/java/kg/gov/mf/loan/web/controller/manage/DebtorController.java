			...
			if (!id.equals("")) {
				...
				if (loanSummary.getRecord_status() == 1 && loanSummary.getLoanSummaryType() == LoanSummaryType.MANUAL) {
					...
					if (loan.getCurrency().getId() != 1) {
						...
						newLoanSummary.setPaidPrincipal(conditional(loanSummary.getPaidPrincipal() * rate));
						newLoanSummary.setPaidInterest(conditional(loanSummary.getPaidInterest() * rate));
						newLoanSummary.setPaidPenalty(conditional(loanSummary.getPaidPenalty() * rate));
						newLoanSummary.setPaidFee(conditional(loanSummary.getPaidFee() * rate));
						newLoanSummary.setTotalOutstanding(conditional(loanSummary.getTotalOutstanding() * rate));
						newLoanSummary.setOutstadingPrincipal(conditional(loanSummary.getOutstadingPrincipal() * rate));
						newLoanSummary.setOutstadingInterest(conditional(loanSummary.getOutstadingInterest() * rate));
						newLoanSummary.setOutstadingPenalty(conditional(loanSummary.getOutstadingPenalty() * rate));
						newLoanSummary.setOutstadingFee(conditional(loanSummary.getOutstadingFee() * rate));
						newLoanSummary.setTotalOverdue(conditional(loanSummary.getTotalOverdue() * rate));
						newLoanSummary.setOverduePrincipal(conditional(loanSummary.getOverduePrincipal() * rate));
						newLoanSummary.setOverdueInterest(conditional(loanSummary.getOverdueInterest() * rate));
						newLoanSummary.setOverduePenalty(conditional(loanSummary.getOverduePenalty() * rate));
						newLoanSummary.setOverdueFee(conditional(loanSummary.getOverdueFee() * rate));
						newLoanSummary.setTotalPrincipalPaid(conditional(loanSummary.getTotalPrincipalPaid() * rate));
						newLoanSummary.setTotalInterestPaid(conditional(loanSummary.getTotalInterestPaid() * rate));
						newLoanSummary.setTotalPenaltyPaid(conditional(loanSummary.getTotalPenaltyPaid() * rate));
						newLoanSummary.setTotalFeePaid(conditional(loanSummary.getTotalFeePaid() * rate));
						newLoanSummary.setOnDate(loanSummary.getOnDate());
						newLoanSummary.setTotalPaidKGS(conditional(loanSummary.getTotalPaidKGS()));
						newLoanSummary.setTotalPaid(conditional(loanSummary.getTotalPaid()));
...
//	choose existing agreement for debtor and select loans
	...
	public Boolean checkHasUnregisteredCollectionPhase(@PathVariable Long debtorId){
		...
		String isThereUnregisteredCollectionPhase = "select if(COUNT(*)>0,'true','false')\n" +
				"from collection_phase_view where v_debtor_id = "+debtorId+" and (v_cph_doc_number is null or v_cph_doc_number = '-') and YEAR(v_cph_startDate) = year(now())";
